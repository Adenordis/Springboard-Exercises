...
SELECT COUNT(name) AS number_free_to_members FROM Facilities 
WHERE membercost = 0
...
SELECT Facilities.name as court_name, CONCAT(Members.firstname, ' ', Members.surname) AS member_name
FROM Bookings
...
SELECT * FROM 
  ...
  LEFT JOIN Facilities ON Facilities.facid = Bookings.facid
  LEFT JOIN Members ON Members.memid = Bookings.memid
  WHERE Bookings.starttime LIKE '2012-09-14%') BookingCosts
WHERE COST > 30
ORDER BY 3 DESC
...
SELECT facility_name, SUM(bookingcost) AS revenue FROM 
  ...
  FROM Bookings
  LEFT JOIN Facilities ON Facilities.facid = Bookings.facid
  LEFT JOIN Members ON Members.memid = Bookings.memid) BookingCosts
GROUP BY facility_name
HAVING SUM(bookingcost) < 1000
ORDER BY 2
